...
public class FloatingIPServiceImpl extends BaseNetworkingServices implements NetFloatingIPService {
     ...
     */
    @Override
    public NetFloatingIP bindQosPolicy(String fipId, NetFloatingIP netFloatingIP) {
        checkNotNull(fipId);
        checkNotNull(netFloatingIP.getQosPolicyId());
        return put(NeutronFloatingIP.class, uri("/floatingips/%s", fipId)).entity(netFloatingIP).execute(ExecutionOptions.create(PropagateOnStatus.on(404)));
    }
